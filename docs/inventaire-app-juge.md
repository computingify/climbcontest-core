...
| D2 | L'application suivait le mode clair/sombre du système. Une salle d'escalade est mal éclairée, et un écran clair éblouit quand on lève les yeux vers le mur. | Sombre, toujours. **⚠️ La PWA a tranché l'inverse à la [spec 039](../specs/039-pwa-claire/spec.md)** : clair par défaut, sombre si le téléphone le demande. Ce n'est pas une contradiction oubliée — c'est le même arbitrage rendu avec l'autre moitié du terrain : la salle d'Annonay a une baie vitrée, une compétition se juge de jour, et un écran sombre lu en plein jour se compense à la luminosité, donc à la batterie. L'app Android, elle, **reste sombre** tant qu'aucune spec ne l'a fait suivre. |
